...
- **`replay_session` builds its history window within one session**, so one-bar-per-session EOD data never accumulates history and any `min_history` above zero skips forever. A daily signal needing prior bars must take them through `extras` (as VRP does). Open roadmap item.
