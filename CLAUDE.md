...
256 tests, none needing a Fyers token or network: the engines, `chain_pricing`, `market_hours`, `recorder`, `snapshot_store`, `eod_vol`, `lot_sizes`, `bhavcopy` (importer end to end on a synthetic Black-76 archive), `signals`/`backtest`, `trading`, `notify`, `alert_engine/percentile_threshold`. `/api/ivrank` is the only router under test (fake broker via monkeypatch + `dependency_overrides`); the rest and `fyers_client` are roadmap item 54.
...
- `backtest/` replays recorded snapshots through the registry. The history window is appended to only *after* evaluation, so look-ahead is structurally impossible rather than merely discouraged. **Every result is reported against a matched null** (random entries at the same clock times, same direction mix) — a signal is not alpha until it beats that, and the verdict strings (`NO_EDGE`, `INSUFFICIENT_DATA`) are deliberately words rather than numbers you can talk yourself into. Horizons in `labels.py` are fixed in advance; never tune them per signal. **Two labelling modes**: intraday (`5m/15m/30m/60m/eod`, null matched on clock time) and daily (`1d/5d/20d` counting trading sessions, null matched on weekday to control for the weekly expiry cycle). `run_backtest` picks the mode from the data — one bar per session means daily — so EOD history cannot be scored on intraday horizons by forgetting a flag; `label_mode` forces it.
...
- **Vol signals are scored by the wrong target.** `backtest/labels.py` measures signed underlying return, which cannot evaluate SHORT_VOL / LONG_VOL: those pay on realised vol versus implied, not on direction. Item 30 (VRP) is built and correct but its NO_EDGE verdict only means it does not predict direction. A vol-outcome labeller is an open roadmap item. `realized_vol.compute_rv_series` is the deprecated undated version — use `rv_series_from_dated_closes` with `spot_history`.
