...
375 tests, none needing a Fyers token or network: the engines, `chain_pricing`, `market_hours`, `recorder`, `snapshot_store`, `eod_vol`, `lot_sizes`, `bhavcopy` (importer end to end on a synthetic Black-76 archive), `signals`/`backtest`, `trading`, `notify`, `alert_engine/percentile_threshold`. Routers are covered by `test_routers.py` (fake broker via monkeypatch + `dependency_overrides`, module-scoped client because the lifespan starts a scheduler); `fyers_client` itself is still untested.
