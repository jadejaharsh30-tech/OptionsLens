    ...
    <div className="flex flex-col gap-5">
      ...
      <InfoPanel title="Understanding the IV Surface + IV Rank">
        ...
        <Section title="The Summary Metrics Explained">
          ...
          <KV label="History Days" value="Trading dates with a 30-day ATM IV reading in the local store. Years of history can be loaded at once from NSE end-of-day files (python -m bhavcopy.importer); after that the backend adds one reading at 15:10 IST each trading day. IV Rank needs 20 dates and is reliable after about a year." />
          <KV label="Current IV" value="30-day ATM implied volatility: the at-the-money IV of the two expiries either side of 30 days, interpolated to exactly 30 days. A fixed tenor removes the swing a single expiry shows as it runs down to expiry, which on weekly contracts is several vol points. This is the single number that summarises 'how expensive' options are right now." />
        </Section>

        <Section title="IV Rank — How to Use It">
          <P>IV Rank tells you where current 30-day IV sits relative to its own history over the past year. It is a percentile: <strong>the share of the last 252 daily readings that were below today's</strong>. It answers "is IV high or low compared to its own recent history?" — which is more useful than the raw IV number alone.</P>
          <P>This is deliberately not the classic (Current IV − 52w Low) ÷ (52w High − 52w Low). Under that formula a single spike day sets the high for a whole year and pins every later reading near zero. A percentile counts days, so one outlier moves it by one day's worth.</P>
